...
import ch.qos.logback.classic.p006db.SQLBuilder;
import ch.qos.logback.classic.p006db.names.DBNameResolver;
import ch.qos.logback.classic.p006db.names.DefaultDBNameResolver;
...
public class SQLiteAppender extends UnsynchronizedAppenderBase<ILoggingEvent> {
    ...
    /* renamed from: db */
    private SQLiteDatabase f61054db;
    ...
    private void insertProperties(Map<String, String> map, long j) throws SQLException {
        if (map.size() > 0) {
            SQLiteStatement compileStatement = this.f61054db.compileStatement(this.insertPropertiesSQL);
            try {
    ...
    private void insertThrowable(IThrowableProxy iThrowableProxy, long j) throws SQLException {
        SQLiteStatement compileStatement = this.f61054db.compileStatement(this.insertExceptionSQL);
        short s = 0;
    ...
    public void append(ILoggingEvent iLoggingEvent) {
        if (isStarted()) {
            try {
                clearExpiredLogs(this.f61054db);
                SQLiteStatement compileStatement = this.f61054db.compileStatement(this.insertSQL);
                this.f61054db.beginTransaction();
                long subAppend = subAppend(iLoggingEvent, compileStatement);
                if (subAppend != -1) {
                    secondarySubAppend(iLoggingEvent, subAppend);
                    this.f61054db.setTransactionSuccessful();
                }
                if (this.f61054db.inTransaction()) {
                    this.f61054db.endTransaction();
                }
                compileStatement.close();
            } catch (Throwable th) {
    ...
    public void start() {
        ...
        try {
            ...
            this.f61054db = SQLiteDatabase.openOrCreateDatabase(databaseFile.getPath(), (SQLiteDatabase.CursorFactory) null);
            z = true;
        } catch (SQLiteException e) {
        ...
        if (z) {
            ...
            try {
                this.f61054db.execSQL(SQLBuilder.buildCreateLoggingEventTableSQL(this.dbNameResolver));
                this.f61054db.execSQL(SQLBuilder.buildCreatePropertyTableSQL(this.dbNameResolver));
                this.f61054db.execSQL(SQLBuilder.buildCreateExceptionTableSQL(this.dbNameResolver));
                clearExpiredLogs(this.f61054db);
                super.start();
                this.started = true;
