...
import com.amazonaws.mobileconnectors.p025s3.transfermanager.internal.CopyCallable;
import com.amazonaws.mobileconnectors.p025s3.transfermanager.internal.CopyImpl;
import com.amazonaws.mobileconnectors.p025s3.transfermanager.internal.CopyMonitor;
import com.amazonaws.mobileconnectors.p025s3.transfermanager.internal.DownloadImpl;
import com.amazonaws.mobileconnectors.p025s3.transfermanager.internal.DownloadMonitor;
import com.amazonaws.mobileconnectors.p025s3.transfermanager.internal.MultipleFileDownloadImpl;
import com.amazonaws.mobileconnectors.p025s3.transfermanager.internal.MultipleFileTransferMonitor;
import com.amazonaws.mobileconnectors.p025s3.transfermanager.internal.MultipleFileUploadImpl;
import com.amazonaws.mobileconnectors.p025s3.transfermanager.internal.S3ProgressListener;
import com.amazonaws.mobileconnectors.p025s3.transfermanager.internal.S3ProgressListenerChain;
import com.amazonaws.mobileconnectors.p025s3.transfermanager.internal.TransferManagerUtils;
...
import com.amazonaws.mobileconnectors.p025s3.transfermanager.internal.UploadCallable;
import com.amazonaws.mobileconnectors.p025s3.transfermanager.internal.UploadImpl;
import com.amazonaws.mobileconnectors.p025s3.transfermanager.internal.UploadMonitor;
...
public class TransferManager {
    ...
    /* renamed from: s3 */
    private final AmazonS3 f68562s3;
    ...
    private Download doDownload(GetObjectRequest getObjectRequest, File file, TransferStateChangeListener transferStateChangeListener, S3ProgressListener s3ProgressListener, boolean z) {
        ...
        ObjectMetadata objectMetadata = this.f68562s3.getObjectMetadata(getObjectMetadataRequest);
    ...
    private Upload doUpload(PutObjectRequest putObjectRequest, TransferStateChangeListener transferStateChangeListener, S3ProgressListener s3ProgressListener, PersistableUpload persistableUpload) throws AmazonServiceException, AmazonClientException {
        ...
        UploadImpl uploadImpl = new UploadImpl("Uploading to " + putObjectRequest.getBucketName() + DEFAULT_DELIMITER + putObjectRequest.getKey(), transferProgress, s3ProgressListenerChain, transferStateChangeListener);
                ...
                try {
                    ...
                    if (ServiceUtils.retryableDownloadS3ObjectToFile(file, new ServiceUtils.RetryableS3DownloadTask() { // from class: com.amazonaws.mobileconnectors.s3.transfermanager.TransferManager.2.1
                        ...
                        public S3Object getS3ObjectStream() {
                            S3Object object = TransferManager.this.f68562s3.getObject(getObjectRequest);
                        ...
                        public boolean needIntegrityCheck() {
                            if (!ServiceUtils.skipMd5CheckPerRequest(getObjectRequest) && !(TransferManager.this.f68562s3 instanceof AmazonS3EncryptionClient)) {
                                return true;
                            }
                            return false;
                        }
    ...
    public void abortMultipartUploads(String str, Date date) throws AmazonServiceException, AmazonClientException {
        MultipartUploadListing listMultipartUploads = this.f68562s3.listMultipartUploads((ListMultipartUploadsRequest) appendSingleObjectUserAgent(new ListMultipartUploadsRequest(str)));
        do {
            for (MultipartUpload multipartUpload : listMultipartUploads.getMultipartUploads()) {
                if (multipartUpload.getInitiated().compareTo(date) < 0) {
                    this.f68562s3.abortMultipartUpload((AbortMultipartUploadRequest) appendSingleObjectUserAgent(new AbortMultipartUploadRequest(str, multipartUpload.getKey(), multipartUpload.getUploadId())));
                }
            }
            listMultipartUploads = this.f68562s3.listMultipartUploads((ListMultipartUploadsRequest) appendSingleObjectUserAgent(new ListMultipartUploadsRequest(str).withUploadIdMarker(listMultipartUploads.getNextUploadIdMarker()).withKeyMarker(listMultipartUploads.getNextKeyMarker())));
        ...
        do {
            ...
            do {
                if (objectListing == null) {
                    objectListing = this.f68562s3.listObjects(new ListObjectsRequest().withBucketName(str).withDelimiter(DEFAULT_DELIMITER).withPrefix(str4));
                } else {
                    objectListing = this.f68562s3.listNextBatchOfObjects(objectListing);
                }
                for (S3ObjectSummary s3ObjectSummary : objectListing.getObjectSummaries()) {
                    if (!s3ObjectSummary.getKey().equals(str4)) {
                        List<String> commonPrefixes = objectListing.getCommonPrefixes();
                        if (!commonPrefixes.contains(s3ObjectSummary.getKey() + DEFAULT_DELIMITER)) {
                            linkedList.add(s3ObjectSummary);
                            j += s3ObjectSummary.getSize();
                        }
                    }
                    Log log2 = log;
                    log2.debug("Skipping download for object " + s3ObjectSummary.getKey() + " since it is also a virtual directory");
                }
    ...
    public void shutdownNow(boolean z) {
        ...
        if (z) {
            AmazonS3 amazonS3 = this.f68562s3;
    ...
    public MultipleFileUpload uploadFileList(String str, String str2, File file, List<File> list, ObjectMetadataProvider objectMetadataProvider) {
        ...
        if (file != null && file.exists() && file.isDirectory()) {
            String str5 = DEFAULT_DELIMITER;
            if (str4 == null || str2.length() == 0) {
                str4 = "";
            } else if (!str4.endsWith(DEFAULT_DELIMITER)) {
                str4 = str4 + DEFAULT_DELIMITER;
            }
            ProgressListenerChain progressListenerChain = new ProgressListenerChain(new ProgressListener[0]);
            TransferProgress transferProgress = new TransferProgress();
            MultipleFileTransferProgressUpdatingListener multipleFileTransferProgressUpdatingListener = new MultipleFileTransferProgressUpdatingListener(transferProgress, progressListenerChain);
    ...
    public Copy copy(CopyObjectRequest copyObjectRequest, TransferStateChangeListener transferStateChangeListener) throws AmazonServiceException, AmazonClientException {
        ...
        ObjectMetadata objectMetadata = this.f68562s3.getObjectMetadata(new GetObjectMetadataRequest(copyObjectRequest.getSourceBucketName(), copyObjectRequest.getSourceKey()).withSSECustomerKey(copyObjectRequest.getSourceSSECustomerKey()));
        TransferProgress transferProgress = new TransferProgress();
        ...
        CopyImpl copyImpl = new CopyImpl("Copying object from " + copyObjectRequest.getSourceBucketName() + DEFAULT_DELIMITER + copyObjectRequest.getSourceKey() + " to " + copyObjectRequest.getDestinationBucketName() + DEFAULT_DELIMITER + copyObjectRequest.getDestinationKey(), transferProgress, progressListenerChain, transferStateChangeListener);
