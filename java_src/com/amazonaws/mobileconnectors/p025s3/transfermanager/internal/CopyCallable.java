...
/* renamed from: com.amazonaws.mobileconnectors.s3.transfermanager.internal.CopyCallable */
...
public class CopyCallable implements Callable<CopyResult> {
    ...
    /* renamed from: s3 */
    private final AmazonS3 f68563s3;
    ...
    public CopyCallable(TransferManager transferManager, ExecutorService executorService, CopyImpl copyImpl, CopyObjectRequest copyObjectRequest, ObjectMetadata objectMetadata, ProgressListenerChain progressListenerChain) {
        this.f68563s3 = transferManager.getAmazonS3Client();
    ...
    private CopyResult copyInOneChunk() {
        CopyObjectResult copyObject = this.f68563s3.copyObject(this.copyObjectRequest);
    ...
    private void copyInParts() throws Exception {
        ...
        try {
            ...
            fireProgressEvent(8);
            try {
                this.f68563s3.abortMultipartUpload(new AbortMultipartUploadRequest(destinationBucketName, destinationKey, this.multipartUploadId));
            } catch (Exception e2) {
                Log log2 = log;
                log2.info("Unable to abort multipart upload, you may need to manually remove uploaded parts: " + e2.getMessage(), e2);
            }
            throw e;
        }
    }
    ...
    private void copyPartsInParallel(CopyPartRequestFactory copyPartRequestFactory) {
        while (copyPartRequestFactory.hasMoreRequests()) {
            if (!this.threadPool.isShutdown()) {
                this.futures.add(this.threadPool.submit(new CopyPartCallable(this.f68563s3, copyPartRequestFactory.getNextCopyPartRequest())));
            } else {
    ...
    private long getOptimalPartSize(long j) {
        ...
        Log log2 = log;
        log2.debug("Calculated optimal part size: " + calculateOptimalPartSizeForCopy);
    ...
    private String initiateMultipartUpload(CopyObjectRequest copyObjectRequest) {
        ...
        String uploadId = this.f68563s3.initiateMultipartUpload(withCannedACL).getUploadId();
        Log log2 = log;
