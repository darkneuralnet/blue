...
public class CrashlyticsReportPersistence {
    ...
    private static final Comparator<? super File> LATEST_SESSION_ID_FIRST_COMPARATOR = new Comparator() { // from class: bE0
    ...
    private static final FilenameFilter EVENT_FILE_FILTER = new FilenameFilter() { // from class: cE0
    ...
    private SortedSet<String> capAndGetOpenSessions(String str) {
        ...
        while (openSessionIds.size() > 8) {
            ...
            logger.m47341d("Removing session over cap: " + last);
    ...
    private void synthesizeNativeReportFile(File file, CrashlyticsReport.FilesPayload filesPayload, String str, CrashlyticsReport.ApplicationExitInfo applicationExitInfo) {
        try {
            ...
            Logger logger = Logger.getLogger();
            logger.m47332w("Could not synthesize final native report file for " + file, e);
        }
    }
    ...
    private void synthesizeReport(String str, long j) {
        ...
        if (sessionFiles.isEmpty()) {
            Logger.getLogger().m47335v("Session " + str + " has no events.");
            return;
        }
        ...
        loop0: while (true) {
            ...
            for (File file : sessionFiles) {
                try {
                    ...
                    Logger.getLogger().m47332w("Could not add event to report for " + file, e);
                }
        ...
        if (arrayList.isEmpty()) {
            Logger.getLogger().m47333w("Could not parse event files for session " + str);
            return;
        }
    ...
    private void synthesizeReportFile(File file, List<CrashlyticsReport.Session.Event> list, long j, boolean z, String str) {
        ...
        try {
            ...
            Logger logger = Logger.getLogger();
            logger.m47332w("Could not synthesize final report file for " + file, e);
        }
    }

    private int trimEvents(String str, int i) {
        List<File> sessionFiles = this.fileStore.getSessionFiles(str, new FilenameFilter() { // from class: ZD0
            @Override // java.io.FilenameFilter
        ...
        Collections.sort(sessionFiles, new Comparator() { // from class: aE0
            @Override // java.util.Comparator
            public final int compare(Object obj, Object obj2) {
    ...
    public void finalizeReports(String str, long j) {
        for (String str2 : capAndGetOpenSessions(str)) {
            Logger logger = Logger.getLogger();
            logger.m47335v("Finalizing report for session " + str2);
    ...
    public void finalizeSessionWithNativeEvent(String str, CrashlyticsReport.FilesPayload filesPayload, CrashlyticsReport.ApplicationExitInfo applicationExitInfo) {
        ...
        Logger logger = Logger.getLogger();
        logger.m47341d("Writing native session report for " + str + " to file: " + sessionFile);
    ...
    public List<CrashlyticsReportWithSessionId> loadFinalizedReports() {
        ...
        for (File file : allFinalizedReportFiles) {
            try {
                ...
                Logger logger = Logger.getLogger();
                logger.m47332w("Could not load report file " + file + "; deleting", e);
                file.delete();
            }
        }
        return arrayList;
    ...
    public void persistReport(CrashlyticsReport crashlyticsReport) {
        ...
        if (session == null) {
            Logger.getLogger().m47341d("Could not get session for report");
            return;
        }
        ...
        try {
            ...
            Logger logger = Logger.getLogger();
            logger.m47340d("Could not persist report for session " + identifier, e);
        }
    }
    ...
    public void persistEvent(CrashlyticsReport.Session.Event event, String str, boolean z) {
        ...
        try {
            ...
            Logger logger = Logger.getLogger();
            logger.m47332w("Could not persist event for session " + str, e);
        }
        trimEvents(str, i);
    }
