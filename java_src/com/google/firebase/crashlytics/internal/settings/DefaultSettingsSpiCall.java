...
public class DefaultSettingsSpiCall implements SettingsSpiCall {
    ...
    private JSONObject getJsonObjectFrom(String str) {
        try {
            ...
            logger.m47332w("Failed to parse settings JSON from " + this.url, e);
            Logger logger2 = this.logger;
            logger2.m47333w("Settings response " + str);
            return null;
        }
    }
    ...
    public JSONObject handleResponse(HttpResponse httpResponse) {
        ...
        Logger logger = this.logger;
        logger.m47335v("Settings response code was: " + code);
        if (requestWasSuccessful(code)) {
            return getJsonObjectFrom(httpResponse.body());
        }
        Logger logger2 = this.logger;
        logger2.m47339e("Settings request failed; (status: " + code + ") from " + this.url);
        return null;
    }
    ...
    public JSONObject invoke(SettingsRequest settingsRequest, boolean z) {
        if (z) {
            try {
                ...
                Logger logger = this.logger;
                logger.m47341d("Requesting settings from " + this.url);
                Logger logger2 = this.logger;
                logger2.m47335v("Settings query params were: " + queryParamsFor);
                return handleResponse(applyHeadersTo.execute());
            } catch (IOException e) {
                this.logger.m47338e("Settings request failed.", e);
                return null;
            }
        }
