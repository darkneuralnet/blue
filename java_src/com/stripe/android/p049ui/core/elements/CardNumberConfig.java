...
public final class CardNumberConfig implements CardDetailsTextFieldConfig {
    ...
    private final int label = C19369R.string.acc_label_card_number;
    private final int keyboard = C39329cl2.f61189b.m60911e();
    private final InterfaceC32735Eu6 visualTransformation = new CardNumberVisualTransformation(' ');
    ...
    public TextFieldState determineState(CardBrand brand, String number, int i) {
        ...
        if (brand.getMaxLengthForCardNumber(number) != -1) {
            z = true;
        } else {
            z = false;
        }
        ...
        if (brand == CardBrand.Unknown) {
            return new TextFieldStateConstants.Error.Invalid(C19369R.string.invalid_card_number, null, 2, null);
        }
        if (z && number.length() < i) {
            return new TextFieldStateConstants.Error.Incomplete(C19369R.string.invalid_card_number);
        }
