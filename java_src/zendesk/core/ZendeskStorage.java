...
class ZendeskStorage implements Storage {
    ...
    private String generateSdkHash(ApplicationConfiguration applicationConfiguration) {
        ...
        return U31.m82035c(String.format(locale, SDK_HASH_FORMAT, applicationConfiguration.getZendeskUrl().toLowerCase(locale), applicationConfiguration.getApplicationId().toLowerCase(locale), applicationConfiguration.getOauthClientId().toLowerCase(locale)));
    }
