...
class ZendeskRequestStorage implements RequestStorage {
    ...
    private void checkForAndMigrateLegacyRequestData() {
        if (!((Boolean) this.memoryCache.getOrDefault(MEMORY_CACHE_MIGRATED_KEY, Boolean.FALSE)).booleanValue()) {
            ...
            if (C43505jm0.m29948i(legacyRequests)) {
    ...
    public boolean isRequestDataExpired() {
        ...
        if (Math.abs(System.currentTimeMillis() - parseLong) > HOUR_IN_MILLIS) {
            z = true;
        } else {
            z = false;
        }
        if (!z) {
            C33694Ix2.m101457b(LOG_TAG, "Returning Read Requests from cache. Cached Read Requests will expire in %s", HOURS_MINUTES_FORMAT.format(Long.valueOf(parseLong)));
        }
        return z;
    }
