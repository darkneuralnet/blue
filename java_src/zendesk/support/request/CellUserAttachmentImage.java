...
public class CellUserAttachmentImage extends CellBase implements CellType.Attachment, CellType.Stateful {
    ...
    public CellUserAttachmentImage(CellBindHelper cellBindHelper, StateMessage stateMessage, StateRequestAttachment stateRequestAttachment, Date date, boolean z, boolean z2, List<StateMessage> list, boolean z3) {
        super(cellBindHelper, C52975zk4.zs_request_user_attachment_image, stateRequestAttachment.getId(), -2147483647L, date);
    ...
    private String buildTalkBackString(Context context) {
        ...
        sb.append(context.getString(C41720gl4.zs_request_message_user_image_accessibility, this.attachment.getName()));
        sb.append(" ");
        if (!this.showError) {
    ...
    public void bind(ComponentRequestAdapter.RequestViewHolder requestViewHolder) {
        ImageView imageView = (ImageView) requestViewHolder.findCachedView(C35181Pg4.request_user_message_attachment_image);
        this.utils.bindImage(imageView, this.attachment);
        this.utils.addOnClickListenerForImageAttachment(imageView, this.attachment);
        this.utils.bindStatusLabel((TextView) requestViewHolder.findCachedView(C35181Pg4.request_user_attachment_image_status), this.lastErrorCellOfBlock, this.markAsDelivered);
        ...
        View findCachedView = requestViewHolder.findCachedView(C35181Pg4.request_user_message_attachment_container);
